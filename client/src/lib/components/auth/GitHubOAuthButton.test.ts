...
const { mockInitiateGitHubOAuth } = vi.hoisted(() => ({
	mockInitiateGitHubOAuth: vi.fn()
}));

// Mock the apiAuth module
vi.mock('$lib/services/apiAuth', () => ({
	initiateGitHubOAuth: mockInitiateGitHubOAuth
}));

import GitHubOAuthButton from './GitHubOAuthButton.svelte';
...
describe('GitHubOAuthButton', () => {
	...
	describe('rendering', () => {
		...
		it('should display translated label', () => {
			...
			expect(screen.getByRole('button')).toHaveTextContent('auth.oauth.github');
		});
	...
	describe('props', () => {
		it('should be disabled when disabled prop is true', () => {
			render(GitHubOAuthButton, { props: { disabled: true } });
			const button = screen.getByRole('button');
		...
		it('should apply custom class', () => {
			render(GitHubOAuthButton, { props: { class: 'my-custom-class' } });
			const button = screen.getByRole('button');
	...
	describe('OAuth flow', () => {
		it('should generate state and initiate OAuth on click', async () => {
			...
			expect(mockInitiateGitHubOAuth).toHaveBeenCalledWith(mockUUID);
		});
