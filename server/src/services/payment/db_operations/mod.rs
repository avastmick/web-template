...
/// Parameters for updating payment after checkout
#[derive(Debug)]
...
pub trait PaymentDbOperations {
    ...
    async fn update_payment_after_checkout(&self, update: &CheckoutUpdate) -> AppResult<()>;
...
impl PaymentDbOperations for PaymentService {
    ...
    async fn update_payment_after_checkout(&self, update: &CheckoutUpdate) -> AppResult<()> {
        ...
        let subscription_start_str = update.subscription_start.map(|dt| dt.to_rfc3339());
        let subscription_end_str = update.subscription_end.map(|dt| dt.to_rfc3339());
        ...
        .bind(update.stripe_payment_intent_id.as_deref())
        ...
        .bind(now)
        .bind(update.payment_id.to_string())
        .execute(&self.db_pool)
        .await?;
